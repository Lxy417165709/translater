...
func (s *State) GetLinesOfLinkInformationFromHere(startId int, StateToId map[*State]int, StateIsVisit map[*State]bool) []string{
	if StateIsVisit[s] {
		return []string{}
	}
	...
	for bytes, nextStates := range s.next {
		for _, nextState := range nextStates {
			result = append(result, nextState.GetLinesOfLinkInformationFromHere(len(StateToId), StateToId, StateIsVisit)...)
			option := string(bytes)
